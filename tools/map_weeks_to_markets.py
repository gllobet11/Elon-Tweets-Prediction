...
def generate_slug_variations(week_start_str: str) -> list[str]:
    ...
    variations.append(f"elon-musk-of-tweets-{month_start}-{day_start}-{month_end}-{day_end}")
    
    # Patrón 3: Con "number-of" en lugar de "of"
    if month_start == month_end:
        variations.append(f"elon-musk-number-of-tweets-{month_start}-{day_start}-{day_end}")
    variations.append(f"elon-musk-number-of-tweets-{month_start}-{day_start}-{month_end}-{day_end}")
...
def test_slug(slug: str, build_id: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            event_data = data.get("pageProps", {}).get("dehydratedState", {}).get("queries", [])
            for query in event_data:
                if "data" in query.get("state", {}) and isinstance(query["state"]["data"], dict):
...
def get_current_build_id() -> str:
    ...
    try:
        ...
        if match:
            ...
            logger.error("BUILD_ID pattern not found on Polymarket homepage. Using fallback.")
...
def map_all_weeks_to_market_ids():
    ...
    # Save the mapping
    project_root_dynamic = os.path.abspath(os.path.join(os.path.dirname(__file__), ".."))
    output_path = os.path.join(project_root_dynamic, "data", "markets", "week_to_market_id_map.json")
