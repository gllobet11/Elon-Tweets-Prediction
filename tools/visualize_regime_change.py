...
def visualize_regime_change():
    ...
    try:
        ...
        monthly_counts = daily_counts_filtered['n_tweets'].resample("MS").sum() # 'MS' para Month Start
        ...
        ax1.plot(monthly_counts.index, monthly_counts.values, marker='o', linestyle='-', label="Tweets por Mes")
        
        # Marcar cambio de régimen si está en el rango visible
        if regime_change_date >= monthly_counts.index.min():
            ax1.axvline(regime_change_date, color='r', linestyle='--', linewidth=2, label=f"Cambio de Régimen ({regime_change_date.date()})")
        ...
        if doge_event_date >= monthly_counts.index.min():
            ax1.axvline(doge_event_date, color='purple', linestyle=':', linewidth=2, label="Evento DOGE (30 May 2025)")
        ...
        ax2.bar(monthly_diffs.index, monthly_diffs.values, color='gray', label="Diferencia Mensual de Tweets")
        ...
        ax2.set_title("Diferencia Mensual en el Número de Tweets (Month-over-Month)", fontsize=16)
