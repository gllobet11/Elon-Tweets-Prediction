...
try:
    ...
    # Get WEEKS_TO_VALIDATE from financial_optimizer as it's a core config for it
    from src.strategy.financial_optimizer import WEEKS_TO_VALIDATE
...
def generate_backtest_predictions(weeks_to_validate: int, end_date=None):
    ...
    print(f"   -> Validating {len(validation_fridays)} weeks from {validation_fridays[0].date()}")
    ...
    df_weekly = df_pred.groupby('week_start_date').agg(
        y_true=('y', 'sum'),
        y_pred=('yhat', 'sum')
    ).dropna()
...
if __name__ == "__main__":
    try:
        ...
        df_performance = generate_backtest_predictions(weeks_to_validate=WEEKS_TO_VALIDATE)
