...
def fetch_and_save_price_histories():
    ...
    map_path = os.path.join(project_root, "data", "markets", "week_to_market_id_map.json")
    ...
    try:
        ...
        logger.error(f"Error decoding JSON from '{map_path}'. The file might be empty or corrupt.")
        return
    ...
    for week, market_id in week_to_market_map.items():
        ...
        logger.info(f"Fetching price history for week {week}, Market ID: {market_id}...")
        ...
        if history:
            ...
            logger.warning(f"  [⚠️] No price history returned for market ID {market_id}.")
            all_price_histories[market_id] = [] # Save empty list to indicate an attempt was made

    # 4. Save all histories to a single file
    output_path = os.path.join(project_root, "data", "markets", "all_price_histories.json")
    try:
