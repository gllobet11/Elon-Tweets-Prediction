...
def analyze_prediction_bias():
    ...
    try:
        ...
        logger.error(
            f"File not found: {history_path}. Please run the pipeline to generate it."
        )
        return
    ...
    if mean_bias > 0:
        logger.success(
            f"CONCLUSION: The model systematically underestimates the number of tweets by an average of {mean_bias:.2f} tweets per week."
        )
        logger.info("This value will be used as the bias correction factor.")
    else:
        logger.warning(
            f"The model seems to be overestimating. Bias: {mean_bias:.2f}. A different approach may be needed."
        )
