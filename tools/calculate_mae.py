...
try:
    ...
    if "y_true" in df.columns and best_model_column in df.columns:
        ...
        print(
            "\nThis means that, on average, the model's weekly predictions during the backtest period were off by about "
            + f"{mae:.2f} tweets."
        )

    else:
        print(
            f"Error: Required columns ('y_true' or '{best_model_column}') not found in the performance file."
        )
