...
def run_data_ingestion():
    ...
    logger.info(f"Last tweet in historical base is from {last_hist_date}. Fetching new tweets since then.")
    
    ingestor = ApiIngestor()
    df_new = ingestor.fetch(start_date=last_hist_date, end_date=datetime.now(timezone.utc))

    if df_new.empty:
        logger.warning("ApiIngestor returned no new tweets. The dataset may already be up-to-date.")
    else:
    ...
    logger.info(f"Unification: {len(df_hist)} historical + {len(df_new) if not df_new.empty else 0} new = {len(df_unified)} total rows.")

    # --- Data Cleaning on the unified frame ---
    # Ensure all dates are consistent timezone-aware objects
    df_unified['created_at'] = pd.to_datetime(df_unified['created_at'], utc=True, errors='coerce')
    ...
    if not df_unified.empty:
         logger.info(f"Final unified data range: {df_unified['created_at'].min().date()} to {df_unified['created_at'].max().date()}")

    df_unified.to_csv(merged_output_path, index=False)
    logger.success(f"--- Unified data saved successfully to '{os.path.basename(merged_output_path)}' ---")
