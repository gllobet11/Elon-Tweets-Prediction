...
def run_step(command, step_name):
    ...
    # Set PYTHONIOENCODING to UTF-8 for the subprocess to handle Unicode characters correctly
    ...
    if rc != 0:
        ...
        logger.error(
            f"Command '{' '.join(full_command)}' failed with return code {rc}."
        )
        sys.exit(1)
    else:
...
def main(args):
    ...
    if args.optimize:
        ...
        run_step(
            ["tools/model_analysis.py", "--task", "tune_hyperparameters"],
            "Hyperparameter Tuning",
        )
        run_step(
            ["tools/model_analysis.py", "--task", "train_with_tuned_hps"],
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run the complete Elon Musk Tweet Prediction Pipeline."
    )
    parser.add_argument(
        "--optimize",
        action="store_true",
