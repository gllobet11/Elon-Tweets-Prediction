...
import pandas as pd
import altair as alt # Needed for Altair charts
...
try:
    ...
    st.success(f"Model '{model_name}' loaded. Using Alpha: **{optimal_alpha:.4f}** and Kelly Frac: **{optimal_kelly:.2f}** (financially optimized).")
    ...
    if selected_week == "Current Week":
        ...
        bankroll = st.sidebar.number_input("Enter your capital (Bankroll $):", min_value=100.0, value=1000.0, step=100.0)

        # --- Statistical Analysis Section ---
        st.subheader("📈 Statistical Activity Analysis")
        st.markdown("Analysis of historical tweet activity to contextualize Elon Musk's behavior.")
        ...
        c1.metric(label=f"Daily Mean ({kpis['current_month_str']})", value=f"{kpis['monthly_mean']:.1f}")
        c2.metric(label="Yesterday vs Mean Deviation", value=f"{kpis['yesterday_val']:.0f}", delta=f"{kpis['deviation']:+.1f}")
        c3.metric(label="Outlier Days (7d)", value=f"{kpis['outlier_days']} days", delta="High Volatility" if kpis['outlier_days'] >= 2 else "Stable")
        ...
        st.caption("The charts show the average tweets per day of the week and the distribution of the total weekly tweet count.")
        
        # --- Prediction and Market Section ---
        st.divider()
        st.subheader("🤖 Market Prediction and Edge Calculation")
        st.markdown("Combines model prediction with real-time market data to identify trading opportunities.")
        ...
        p1.metric("Total Hybrid Prediction", f"{pred_metrics['weekly_total_prediction']:.2f}")
        p2.metric("Actual Tweets Counted", f"{pred_metrics['sum_of_actuals']}")
        p3.metric("Predicted Tweets (future)", f"{pred_metrics['sum_of_predictions']:.2f}", delta=f"{pred_metrics['remaining_days_fraction']:.2f} remaining days")
        st.caption("Hybrid prediction combines tweets already published this week with the model's prediction for the remaining days.")
        ...
        st.caption("The table compares the probability calculated by the model with the market price. 'Edge' is the difference between the two. 'Bet Size' is the recommended investment according to the fractional Kelly Criterion.")
        
        # 5. Distribution Visualization
        st.divider()
        st.subheader("📊 Probability Distribution Visualization")
        st.markdown("Visual comparison of the model's probability distribution (line) against market-implied probabilities (bars).")
        
        final_chart = chart_generator.generate_probability_comparison_chart(df_opportunities)
        st.altair_chart(final_chart, use_container_width=True)
        st.caption("Areas where the line (model) is above the bars (market) represent a positive 'edge', suggesting a 'buy' bet.")
        ...
        selected_week_dt = pd.to_datetime(selected_week).tz_localize('UTC') # Fix for TypeError
        week_data = historical_performance_df.loc[selected_week_dt]
        
        if not week_data.empty:
            col_hp1, col_hp2 = st.columns(2)
            with col_hp1:
                st.metric(label="Model Prediction (y_pred)", value=f"{week_data['y_pred']:.2f}")
            with col_hp2:
                st.metric(label="Actual Tweets (y_true)", value=f"{week_data['y_true']:.0f}")
            ...
            if not daily_data_for_selected_week.empty:
                historical_chart = chart_generator.generate_historical_week_chart(daily_data_for_selected_week)
                st.altair_chart(historical_chart, use_container_width=True)
            else:
                st.info("No daily tweet activity data available for this historical week.")
