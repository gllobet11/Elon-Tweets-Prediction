    ...
    def _find_and_click_market_button(self, page):
        ...
        for i in range(count):
            ...
            if all(keyword in container_text for keyword in self.keywords):
                ...
                candidates = [
                    container.locator("button[title='Export all tweets in this period']"),
                    btn_by_icon,
                ...
                if not target_btn:
                    logger.warning("   -> Botón no visible inmediatamente. Esperando renderizado...")
                    try:
                        btn_by_icon.first.wait_for(state="visible", timeout=3000)
                        target_btn = btn_by_icon.first
                        logger.info("   -> Botón apareció tras espera.")
                    except:
                        logger.error("❌ El botón no apareció. Dump del HTML de la fila:")
                ...
                if target_btn:
                    ...
                    try:
                        ...
                        destination_path = os.path.join(self.download_path, download.suggested_filename)
                        download.save_as(destination_path)
                        
                        logger.success(f"📦 ¡Éxito! Archivo guardado: {destination_path}")
                        return True
