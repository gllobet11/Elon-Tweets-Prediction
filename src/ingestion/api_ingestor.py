    ...
    def fetch(self, start_date: datetime, end_date: datetime) -> pd.DataFrame:
        ...
        try:
            with httpx.Client() as client:
                response = client.get(self.API_ENDPOINT, params=params, headers=self.HEADERS, timeout=self.timeout)
            ...
            if 'id' in df.columns and 'platformId' in df.columns:
                logger.debug("Found both 'id' and 'platformId' columns. Dropping 'id' (database ID).")
                df = df.drop(columns=['id'])
            elif 'id' in df.columns and 'platformId' not in df.columns:
                 logger.warning("No 'platformId' found in API response, using 'id' as default.")
            # --- End FIX ---
            ...
            logger.success(f"Successfully processed {len(df)} tweets. Dates: {df['created_at'].min().date()} to {df['created_at'].max().date()}.")
            return df
