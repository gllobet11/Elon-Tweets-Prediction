...
class InferenceModel:
    ...
    def train(self, train_df: pd.DataFrame, feature_cols: list, verbose: bool = True):
        ...
        model = sm.GLM(y_train, X_train_const, family=NegativeBinomial(alpha=self.alpha))
    ...
    def predict(self, features: pd.DataFrame):
        ...
        if self.model_res is None:
            raise RuntimeError("❌ El modelo no ha sido entrenado. Llama al método .train() primero.")
