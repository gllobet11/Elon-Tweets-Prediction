...
def get_hybrid_prediction(prophet_model, market_start_date, market_end_date, df_tweets):
    ...
    actuals_mask = (df_tweets["created_at"] >= market_start_date) & (
        df_tweets["created_at"] < now_utc
    )
    ...
    if remaining_days_fraction > 0:
        logger.info(
            f"🔮 Quedan {remaining_days_fraction:.2f} días fraccionales por predecir.",
        )
        ...
        if model_regressors:
            for regressor in model_regressors:
                if regressor in latest_features:
                    ...
                    logger.info(
                        f"   -> Añadiendo regresor '{regressor}' con valor constante: {regressor_value:.4f}",
                    )
                else:
                    # Fallback por si un regresor no se pudo calcular (aunque no debería pasar).
                    future_df[regressor] = 0.0
                    logger.warning(
                        f"   -> No se encontró el regresor '{regressor}' en las features. Usando 0 como fallback.",
                    )
        # --- FIN DEL FIX ---
        ...
        logger.info(
            f" -> Tasa de predicción diaria del modelo: {avg_daily_prediction_rate:.2f} tweets/día",
        )

        # 5. Estimar los tweets para el período restante
        sum_of_predictions = avg_daily_prediction_rate * remaining_days_fraction
        logger.info(
            f" -> Suma de tweets predichos (fraccional): {sum_of_predictions:.2f}",
        )
    else:
