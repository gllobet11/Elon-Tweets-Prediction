...
def load_data():
    ...
    df_tweets['created_at'] = pd.to_datetime(df_tweets['created_at']).dt.tz_convert(None)
    return df_tweets
...
st.markdown("Dashboard diseñado para detectar **sobrecalentamiento** y oportunidades de regresión a la media.")
...
try:
    ...
    month_data = daily_data[(daily_data.index.month == current_month) & (daily_data.index.year == current_year)]
    ...
    band = base.mark_area(opacity=0.2, color='gray').encode(
        y='Lower:Q',
        y2='Upper:Q'
    )
    
    # Puntos Outlier (Rojos)
    outliers = base.mark_circle(color='red', size=60).encode(
        y='Tweets:Q',
        tooltip=['Fecha', 'Tweets']
    ).transform_filter(
        alt.datum.Tweets > alt.datum.Upper
    )
    ...
    with col_A:
        ...
        current_week_start = df_cum.index.max() - pd.Timedelta(days=df_cum.index.max().weekday())
        ...
        last_week_data = df_cum[(df_cum.index >= last_week_start) & (df_cum.index < current_week_start)].copy()
        ...
        avg_week = hist_data.groupby('weekday')['n_tweets'].mean().cumsum().reset_index()
        ...
        # Gráfico de Líneas
        chart_cum = alt.Chart(combined_chart_data).mark_line(point=True).encode(
            x=alt.X('weekday', title='Día de la Semana', axis=alt.Axis(labelExpr="datum.value == 0 ? 'Lun' : datum.value == 1 ? 'Mar' : datum.value == 2 ? 'Mié' : datum.value == 3 ? 'Jue' : datum.value == 4 ? 'Vie' : datum.value == 5 ? 'Sáb' : 'Dom'")),
            y=alt.Y('cumsum', title='Tweets Acumulados'),
            color=alt.Color('Type', scale=alt.Scale(domain=['Semana Actual', 'Semana Anterior', 'Promedio (3 meses)'], range=['red', 'gray', 'blue'])),
            tooltip=['Type', 'Día', alt.Tooltip('cumsum', format='.0f')]
        ).properties(
            title="Carrera Semanal: Acumulado vs Histórico",
            height=350
        )
    ...
    with col_B:
        ...
        chart_bar = alt.Chart(seasonality).mark_bar().encode(
